...
// 1. Swap axios for your new custom api file
import api from '../api'; 
...
const AdminPanel = () => {
  ...
  const fetchData = async () => {
    try {
      // 2. Use the api tool - notice how much cleaner the URLs are
  ...
  const approveUser = async (userId, status) => {
    try {
      // 4. Simplified approval request
      await api.put(`/auth/users/approve/${userId}`, { status });
      alert(`User ${status}!`);
      fetchData();
    } catch {
  ...
  // ... (rest of the styling functions roleBadge, tabStyle, etc. stay exactly the same)
