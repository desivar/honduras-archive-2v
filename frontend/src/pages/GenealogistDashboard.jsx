...
const GenealogistDashboard = ({ onLogout }) => {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        // 🟢 FIXED: Dashboard usually returns the object directly, not .items
  ...
  const handleLogout = async () => {
    try {
      ...
      // 🟢 FIXED: Logout needs a POST request, and we pass the session index
      await api.post('/auth/logout', { sessionIndex: parseInt(sessionIndex) });
    } catch (err) {
  ...
  const removeBookmark = async (recordId) => {
    try {
      // 🟢 FIXED: Added missing backticks and closed the function correctly
      await api.post(`/auth/activity/bookmark/${recordId}`);
  ...
  const saveNote = async (recordId) => {
    try {
      // 🟢 FIXED: Swapped axios for api tool
      await api.post(`/auth/activity/note/${recordId}`, { text: noteText });
