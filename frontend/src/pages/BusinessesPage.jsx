...
// 1. Swap axios for your central API tool
import api from '../api'; 
...
const BusinessCard = ({ record, onDeleteSuccess }) => {
  ...
  const handleDelete = async (e) => {
    ...
    if (window.confirm(`Are you sure you want to delete "${record.businessName}"?`)) {
      try {
        // 2. No need to manually grab token or set headers! api.js does it.
        await api.delete(`/archive/${record._id}`);
...
// ... badgeStyle, citeBtnStyle, etc. ...
const badgeStyle = { backgroundColor: '#f0ede3', border: '1px solid #ACA37E', borderRadius: '20px', padding: '3px 10px', fontSize: '0.82rem', color: '#555' };
const citeBtnStyle = { width: '100%', padding: '10px', backgroundColor: '#586379', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' };
const editBtnStyle = { flex: 1, padding: '9px', backgroundColor: '#737958', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' };
const deleteBtnStyle = { flex: 1, padding: '9px', backgroundColor: '#a94442', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' };
